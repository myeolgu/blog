...
- 사용자가 "커밋 내용"을 요청하면 `commit` 스킬을 불러와 그 지침에 따라 안내한다. 이 경우에는 메시지만 제안하고 직접 커밋하지 않는다.
- 파일을 수정하는 작업을 마치고 변경 범위에 맞는 검증까지 끝낸 뒤에는, 사용자가 따로 요청하지 않아도 자동으로 커밋하고 푸시까지 진행한다.
  - 커밋 메시지는 `commit` 스킬의 형식(`type(scope): subject`, 한글 subject)을 따른다.
  - 검증에 실패했거나 확인하지 못한 항목이 있으면 커밋·푸시하지 않고 사유를 알린다.
